...
function displayRecommendations() {
    ...
    if (recs.length === 0) {
        container.innerHTML = `
            <div class="recommendations-empty">
                ...
                <p>Add more places or try another category</p>
            </div>
        `;
        return;
    }
    ...
    recs.forEach((rec, index) => {
        ...
        card.querySelector('.btn-add-to-trip')?.onclick = () => addRecommendationToTrip(rec);
        card.querySelector('.btn-view-details')?.onclick = () => showRecommendationDetails(rec);
    });
}
...
function displayDayPlans() {
    ...
    if (recommendationsState.dayPlans.length === 0) {
        container.innerHTML = `
            <div class="recommendations-empty">
    ...
    container.innerHTML = recommendationsState.dayPlans
        .map(plan => createDayPlanCard(plan))
        .join('');
}

function createDayPlanCard(plan) {
    const placesHTML = plan.places.map((place, i) => `
